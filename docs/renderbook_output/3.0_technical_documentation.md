...
- **4 - Select Facility or Create New Team**: _If you are a **team member**_: Select your existing team from the dropdown menu. _If you are a **team lead**_: Select Create New Team. 
  ...
  - Click the Submit button. The submitted team name is saved even though the dropdown field remains unchanged.
...
The following message will appear for a few seconds in the upper right-hand corner of your browser window:
...
- **Note:** The login dropdown only shows worlds in which you are a team lead or participant (not an Administrator).
- **Note:** If you have the Administrator role in any team and have been added into "administrator_login" in Epicenter, you will be allowed to login using the "administrator_login" choice. Upon logging in, Administrators are directed to the [Administrator Dashboard](#sim_ui_administrator_dashboard).
...
**Tip:** Refer to this one-page [Sim UI Cheatsheet](https://raw.githubusercontent.com/lzim/mtl/gh-pages/images/mtl_how_3.7_sim_ui_cheatsheet.png) when familiarizing yourself with the Home Page and Dynamic Decision Dashboard. 
...
7. **News:** Click for the latest news about _Modeling to Learn_. An alert will appear if you have unread news items.

8. **Report Bug:** Click to report a bug.

9. **Tutor:** Click any Tutor icon for more information about the section. 

10. **Admin Dashboard:** Click to return to the Administrator Dashboard.
...
15. **Team Data window:** This section allows you to delete, upload, and sort team data files (which will then show in the Start a Session window). Legacy team data files are located under the _MTL_ 3.0 gray bar.
...
**Note:** If there is not a team data file in the dropdown, follow [these steps](#upload-team-data-files-from-the-home-page).
...
- **4 - Play**: Enter the main Sim UI page to see the system story and run experiments.
...
- Care Decision Meters are for _MTL_ Blue teams only.
...
**Tip:** Save this [Sim UI cheatsheet](https://github.com/lzim/mtl/blob/master/blue/session05/s05_learner/mtl_how_3.7_sim_ui_cheatsheet.pdf) to use when running an experiment.
...
7. **Select Team** - Use this dropdown to select your desired team. It enables you to monitor their experiments and navigate to the team's Home page.
...
**Note: Only administrators and the internal team have access to these functions.** Field users can upload their data file(s) from the Dynamic Decision Dashboard (as well as administrators and the internal team). Field users should refer to the [Dynamic Decision Dashboard manual section](#dynamic-decision-dasboard).
...
The following screenshot will orient you to general features of the Maintenance Dashboards.
...
**You can verify you are on the right page, because the URL will say "va-psd-sim/qtipadmin.html" if you are in PROD Tips and "va-psd-demo/qtipadmin.html" if you are in the DEMO Tips.**
