...
> should there be mention here of the different worlds it sets up automatically: _team, _ind, and _shared
...
- Click on the Clinic Selection tab.
...
- Click Apply.
- **Notes:** 
  - If you wish to have quick access to these Clinic Selections in the future, you can make a personal bookmark to return to it. Go to the Bookmarks drop-down menu and click on Add a personal bookmark. In the bookmark name field, use Team PSD convention for the name: Sta3n_Division Name_Date in yyyy_mm_dd format (e.g., 612_redding_2023_08_22 ), then select Save.

> screenshot of bookmark -&- is above example OK to include?

   - Clinics and data not populating? Request and verify your local server views (see section titled Request and Verify Local Server View Permissions).

3.	Scroll to the bottom until you see the View Clinic Selections tab on the left-hand side.
4.	Hover over the Clinic Selections heading and click on the ellipsis that appears.
5.	Click Export Data > Data with Current Layout > Export.
6.	Repeat steps 5 and 6 with Team Flow Selections.
7.	Save using our Team naming convention and post the file  on the shared Team channel for Team members to view and confirm selections. 

**Tips & Tricks:**

> include link to or screenshot of data_cheatsheet?
...
> steps 8 & 9 & assoc'd T&Ts not so much about Clinic Selections; remove?

8.	Click to view the vizDiag tab which shows team trends.
...
Note: If you need access to all sites (e.g., national), click [here](https://dvagov.sharepoint.com/sites/vhaacademicdetailing/SitePages/How-To-Request-Access-to-Data-Tools.aspx) and follow the steps in under “Request Access to Data Tools.”

- To pull patient and clinic level data, you need LSV permissions. Check which facilities you have LSV access for here through VA intranet.
...
1.	Click here to access the OI&T Electronic Permission Access System.
2.	Verify VHA NDS Access Form for Health Operations is selected in the Select Document dropdown. 
3.	Click Create Document. A form with fields will appear.

> insert screenshot of form with numbered pins

4.	Fill in the form:

Note: the following fields should be pre-populated: Full Legal Name, Job Title, Email Address, Active Directory Name with Domain, and Work Telephone Number.
- **Employee Agency**: Veterans Health Administration 
- **Employee Type**: VA Employee (yes, for all of you, none of you are “contractors”). New fields will appear.
- **Contractor Name**: Your name.
- **Contractor Business Address**: Enter your address here, using the fields provided.
- **Contractor Business Phone Number**: Your business phone number.
- **Station Number**: 640 VA Palo Alto Health Care system (for everyone, but the QIICs, who should choose their own station).
- **Supervisor**: Zimmerman, Lindsey E. (for everyone, but the QIICs, can use myself as the supervisor for this access, or use Cliff – Cliff or I will receive an email and need to approve the request)
- **Purpose of Use**: Healthcare Operations
- **Data Sources**: Select Corporate Data Warehouse (CDW)
- **Duration of Access**: Select “I will  need access during the duration of my position.” from the dropdown. 
- **Affirmation**: Check the box.
  
5.	Click the Save and Submit button. A popup will appear. Click OK.

> note to LM: check above step when taking screenshots and revise if needed

6.	Select CDW SQL Datasets and the menu below will appear.
...
8.	Click on the Comments tab. Copy/paste in the following rationale provided to you by the team: 
...
> revise bullet point and screenhshot below depending on DEV work in progress:
